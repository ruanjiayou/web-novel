...
import {
  ...
  createRouteProvider,
  createNaviProvider,
  createMusicPlayerProvider,
  createDebugProvider,
  createSpeakerProvider,
} from 'contexts';
      ...
      <NaviContext.Provider value={navi}>
        <Observer>
          {() => {
            ...
            if (
              !store.app.isLogin &&
              props.location.pathname.startsWith('/novel') &&
              store.app.forceLogin
            ) {
              return <Redirect to={'/novel/auth/login'}></Redirect>;
            } else if (
              store.app.config.isLockerOpen &&
              store.app.config.isLockerLocked
            ) {
              return <LockerView />;
            }
            return (
                  ...
                  {local.layers.map((layer, i) => {
                    ...
                    return (
                      <CSSTransition
                        key={i}
                        timeout={{
                          enter: !router.userEvent ? 0 : 300,
                          exit: !router.userEvent ? 0 : 300,
                        }}
