...
export default ({
  ...
  const local = useLocalStore(() => ({
    ...
    computeInitialOffset() {
      ...
      local.initialOffset.x =
        -Math.abs(element.current.offsetWidth * k - width) / 2;
      local.initialOffset.y =
        -Math.abs(element.current.offsetHeight * k - height) / 2;
    },
    // 更新偏移坐标
    resetOffset() {
  ...
  const _sanitizeOffset = useCallback(() => {
    ...
    let elWidth =
      element.current.offsetWidth * getInitialZoomFactor() * local.zoomFactor;
    let elHeight =
      element.current.offsetHeight * getInitialZoomFactor() * local.zoomFactor;
  ...
  const zoomOutAnimation = useCallback(() => {
    ...
    let center = {
      x:
        -1 * local.offset.x -
        (local.offset.x - local.initialOffset.x) / (1 / local.zoomFactor - 1),
      y:
        -1 * local.offset.y -
        (local.offset.y - local.initialOffset.y) / (1 / local.zoomFactor - 1),
    };
    let updateProgress = (progress) => {
      scaleTo(
        startZoomFactor + progress * (_zoomFactor - startZoomFactor),
        center,
      );
    };
  ...
  const sanitize = useCallback(() => {
    if (local.zoomFactor < 1.3) {
      ...
      if (
        sanitizeOffset.x !== local.offset.x ||
        sanitizeOffset.y !== local.offset.y
      ) {
    ...
    <Observer>
      {() => (
        ...
        >
          ...
          >
            zoom:{local.zoomFactor} scale:{local.lastScale} center.x:
            {lastDragPosition.current ? lastDragPosition.current.x : 0}{' '}
            center.y:{lastDragPosition.current ? lastDragPosition.current.y : 0}
          </span>
          <div
            ref={element}
            style={{
