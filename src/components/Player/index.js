...
export default function ({
  ...
  const onKeyPress = function (e) {
    switch (e.keyCode) {
      ...
      case 38:
        ...
        if (hlsRef.current) {
          local.volume = (
            hlsRef.current.volume + 0.1 > 1 ? 1 : hlsRef.current.volume + 0.1
          ).toFixed(2);
        }
        if (ref.current) {
          local.volume = (
            ref.current.volume + 0.1 > 1 ? 1 : ref.current.volume + 0.1
          ).toFixed(2);
        }
        if (flvRef.current) {
          local.volume = (
            flvRef.current.volume + 0.1 > 1 ? 1 : flvRef.current.volume + 0.1
          ).toFixed(2);
        }
      ...
      case 40:
        if (hlsRef.current) {
          local.volume = (
            hlsRef.current.volume - 0.1 < 0 ? 0 : hlsRef.current.volume - 0.1
          ).toFixed(2);
        }
        if (ref.current) {
          local.volume = (
            ref.current.volume - 0.1 < 0 ? 0 : ref.current.volume - 0.1
          ).toFixed(2);
        }
        if (flvRef.current) {
          local.volume = (
            flvRef.current.volume - 0.1 < 0 ? 1 : flvRef.current.volume - 0.1
          ).toFixed(2);
        }
        setVolume(local.volume);
        local.takePeek(Math.round(local.volume * 100) + '%');
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                width: local.percent + '%',
                backgroundColor: 'rgb(22, 147, 255)',
              }}
            ></div>
          </div>
        )} */}
      ...
      >
        ...
        <AlignSide
          style={{
            ...
            top: 0,
            width: '100%',
            padding: local.isVertical ? 0 : padding,
            zIndex: 2,
            background: local.showControl
              ? 'linear-gradient(-180deg, #333, transparent)'
              : '',
          }}
        >
          {header(
            local.showControl && !local.fullscreen ? (
              <div>
                <Icon src={require('theme/icon/airplay.svg')} />
                <Icon
                  style={{ width: 16 }}
                  src={require('theme/icon/feedback.svg')}
                  onClick={() => {
          ...
          >
            ...
            <VisualBoxView visible={local.showControl}>
              <Icon
                ...
                src={
                  local.paused
                    ? require('../../theme/icon/play-fill.svg')
                    : require('../../theme/icon/suspended-fill.svg')
                }
              />
            </VisualBoxView>
            {/* 下一集 */}
            <VisualBoxView visible={resource.children.length > 1}>
              <FullHeightFix>
                <Icon
                  onClick={playNext}
                  style={{ width: 16, height: 15, marginLeft: 0 }}
                  src={require('../../theme/icon/next.svg')}
                />
              </FullHeightFix>
            </VisualBoxView>
            {/* 进度条 */}
            <FullWidthAuto
              ...
              onClick={(e) => {
                ...
                let time =
                  local.duration * (ne.offsetX / e.currentTarget.offsetWidth);
                onSeek(time);
              }}
            >
              <div
            ...
            <FullWidthFix>
              <Icon
                ...
                src={require('theme/icon/fullscreen.svg')}
              />
            </FullWidthFix>
          </FullWidth>
        </VisualBoxView>
  ...
  const renderMoreLayer = function () {
    return (
      <VisualBoxView visible={local.showMore}>
        <FullHeight
  ...
  const renderEndedLayer = function () {
    return (
      <VisualBoxView visible={local.isEnded}>
        <FullHeight
          style={{
            color: 'white',
          ...
          <AlignCenterXY style={{ flex: 2, height: 'auto', width: '100%' }}>
            {next && (
              <ResourceItem
                item={next}
                onClick={() => {
                  router.replaceView('VideoInfo', { id: next.id });
                }}
              />
            )}
          </AlignCenterXY>
  ...
  return (
    <Observer>
      {() => (
        ...
        >
          {renderVideoLayer()}
          {renderControlLayer((child) => (
            <Navi
              title={
                !local.isVertical && local.showControl ? resource.title : null
              }
              showBack
              wrapStyle={{
                flex: 1,
            ...
            >
              ...
              >
                ...
                <MIconView
                  ...
                  onClick={() => (local.showFeedback = false)}
                />
                <Button
                  type="primary"
                  size="small"
                  style={{ width: 80, margin: '0 auto' }}
                  loading={local.isFeedbacking}
                  onClick={async () => {
