...
const GetPullToRefreshlData = ({
  ...
  const emptyView = renderEmptyView(loader, null, refresh);
  useEffect(() => {
              ...
              {local.trigerTopLimit ? (
                ...
                <div
                  style={{
                    ...
                    height: local.showFinished ? 30 : local.topHeight,
                    overflow: 'hidden',
                  }}
                >
                  {local.showFinished ? '已完成' : '刷新↓'}
                </div>
              )}
              {loader.items.map((item, index) => (
                <div key={index} style={itemWrapStyle}>
                  {renderItem(item, loader.name, index)}
                </div>
              ))}
              {children}
            </div>
