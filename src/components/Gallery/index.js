...
export default ({ items, onChangeIndex, activeIndex = 0, onDoubleTap, onGestureEnd, children }) => {
  ...
  const updateVisibleData = (lastIndex, latestIndex, data) => {
    ...
    container.current.style.setProperty('transform', `translateX(${_offsetX}px)`)
    ...
    const onChange = () => {
      ...
      orientationTimer.current = setTimeout(() => {
        ...
        switch (window.orientation) {
          case 90:
          case -90: viewSize = Math.max(window.innerWidth, window.innerHeight)
    ...
    if (touches.length === 1) {
      if (local.firstMove) {
        ...
        if (isSwiping) {
          ...
          container.current.style.setProperty('transform', `translateX(${offset}px)`)
  ...
  const touchEnd = useCallback(e => {
    ...
    local.index = _index
    container.current.style.setProperty('transform', `translateX(${-(_index * window.innerWidth)}px)`)
    distanceX.current = 0
  ...
  const transitionEnd = useCallback(e => {
    ...
    let _dataIndex = local.data.findIndex(item => item.id === currentData.id)
    local.status = 'end'
  ...
  const renderItem = (item, index) => {
    let image = item.data
    return <PinchZoom style={{ height: '100%' }} key={index} onGestureEnd={onGestureEnd} onDoubleTap={onDoubleTap}>
      <img src={image.path} />
    </PinchZoom>
  }
  return <Observer>{() => (
    <div ref={root} onTransitionEnd={transitionEnd} onTouchEnd={touchEnd} onTouchStart={touchStart} onClick={() => {
      if (local.fullscreen === false) {
        local.fullscreen = true
      }
    }} style={{ width: '100%', height: '100%', position: local.fullscreen ? 'fixed' : 'static', left: 0, top: 0, zIndex: 2, overflow: 'hidden' }}>
      <div ref={container} style={{ transition: local.status !== 'end' && local.status !== 'init' ? 'transform 0.35s cubic-bezier(0.15, 0.3, 0.25, 1) 0s' : 'none', display: 'flex', flex: 1, width: '100%', height: '100%' }}>
        {local.visibleData.map((item, _index) => {
          return <div data-id={item.id} key={item._index} style={{ width: '100%', flexShrink: 0 }}>
