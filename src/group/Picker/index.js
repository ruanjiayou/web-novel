...
export default function Picker({ self }) {
  return <Observer>{() => (
    <div>
      <div className="dd-common-alignside" style={{ borderLeft: '5px solid #ff9999', padding: '10px 0 10px 10px', margin: '10px 0 10px 10px', fontSize: 16 }}>
      ...
      <div onScroll={e => {
        ...
        {self.data.map((d, index) => (<div key={d.id} style={{ margin: 10 }}>
          <ResourceItem item={d} />
        </div>))}
      </div>
