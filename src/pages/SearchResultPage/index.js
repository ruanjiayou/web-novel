  ...
  return <Observer>{() => <FullHeight>
    <FullWidth style={{ height: 50, marginLeft: 10 }}>
      ...
      <FullWidthAuto style={{ marginLeft: 10, position: 'relative', display: 'flex', backgroundColor: '#ccc', alignItems: 'center', border: '0 none', width: '100%', borderRadius: 5, padding: '5px 8px', }}>
        ...
        <input defaultValue={params.search} autoFocus ref={ref => iRef.current = ref} style={{ backgroundColor: '#ccc', height: 30, marginLeft: 10, flex: 1, boxSizing: 'border-box', border: '0 none', borderBottom: '1px solid #999' }}
          onKeyDown={e => {
            if (e.keyCode === 13) {
              const str = iRef.current ? iRef.current.value : '';
              if (str) {
                storage.setValue('historyWords', (storage.getValue('historyWords') || '').split(',').filter(s => s !== '').join(','))
              }
