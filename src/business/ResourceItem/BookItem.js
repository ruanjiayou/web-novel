...
export default function ({ item }) {
  ...
  return <Observer>
    {() => {
      return <Fragment>
        <div className="full-width" style={{ padding: 10 }} onClick={() => { router.pushView('book-info', { id: item.id }) }}>
          ...
          <div className="full-width-auto full-height">
            ...
            <div style={{ color: 'rgb(146, 145, 145)' }}>{timespan(new Date(item.last.updatedAt || Date.now()))} · {item.last.title || '最新章节'}</div>
          </div>
        </div>
      </Fragment>
    }
